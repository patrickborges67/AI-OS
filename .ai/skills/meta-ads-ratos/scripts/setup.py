...
def load_env_file():
    """Carrega variaveis do .env compartilhado ou local legado, se existir."""
    skill_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    skills_dir = os.path.dirname(skill_dir)
    loaded = None
    for env_path in [os.path.join(skills_dir, ".env"), os.path.join(skill_dir, ".env")]:
...
def check_token():
    ...
    if not token:
        ...
        print("           Adicione ao .ai/skills/.env:")
...
def main():
    ...
    print("=" * 55)
    if env_file:
        print(f"  .env local carregado: {env_file}")
