...
Checar se existe `.ai/skills/.env`. Se NAO existir, criar com o template:
...
O token fica isolado no `.ai/skills/.env` e nao vaza pra outras sessoes do terminal.
...
Depois que o `.ai/skills/.env` estiver preenchido e o `setup.py` passar, o Claude DEVE proativamente guiar o cadastro de contas:
