...
# GA4_CLIENT_SECRET=""
# GA4_REFRESH_TOKEN=""
# Ou deixe em branco e o script busca automaticamente no .ai/skills/.env.
```
...
**Modo 2 (OAuth2):** Se o usuario ja tem `google-ads-ratos` configurado com OAuth, os scripts buscam automaticamente as credenciais de `.ai/skills/.env`. Isso so funciona se o refresh token tambem tiver o escopo `https://www.googleapis.com/auth/analytics.readonly`. Tokens gerados apenas com escopo Google Ads (`adwords`) retornam `invalid_scope` na GA4 API. Nesse caso, usar Service Account ou gerar OAuth proprio com escopo Analytics.

### 3. Cadastro de propriedades (contas.yaml) — SETUP CONVERSACIONAL

Depois que o `.ai/skills/.env` estiver preenchido, o Codex DEVE proativamente guiar o cadastro:
...
**Arquivo:** `.ai/skills/ga4-ratos/contas.yaml`
...
Todos os scripts estao em `.ai/skills/ga4-ratos/scripts/`. O padrao e:
