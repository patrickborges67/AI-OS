...
### 2. Verificar .env compartilhado

Checar se existe `.ai/skills/.env` na raiz do projeto. Se NAO existir, criar com o template:
...
**Fallback**: Se existir `.ai/skills/google-ads-ratos/google-ads.yaml`, o SDK carrega dele automaticamente.

### 3. Preencher credenciais no .env

O usuario precisa obter e preencher no `.ai/skills/.env` (tutorial completo em ratosdeia.com.br/assets/tutorial-token-google-ads/):
...
Depois que CLIENT_ID, CLIENT_SECRET e DEVELOPER_TOKEN estiverem no `.ai/skills/.env`, rodar:

```bash
python .ai/skills/google-ads-ratos/scripts/setup.py oauth
```

Isso abre o browser, o usuario autoriza, e o refresh token e salvo automaticamente no `.ai/skills/.env`. Sem copiar/colar nada.

**Ou rodar o fluxo completo de uma vez:**

```bash
python .ai/skills/google-ads-ratos/scripts/setup.py full
```
...
Depois que o `.ai/skills/.env` estiver preenchido e o teste passar, o Codex DEVE proativamente guiar o cadastro de contas:
...
**Arquivo:** `.ai/skills/google-ads-ratos/contas.yaml`

Antes de executar qualquer operacao, o Codex DEVE ler este arquivo para resolver nomes de clientes para IDs.
...
Todos os scripts estao em `.ai/skills/google-ads-ratos/scripts/`. O padrao e:

```
python3 <script>.py <subcomando> [argumentos]
```

O Codex deve interpretar o pedido do usuario e executar o script correto via terminal.
...
**Arquivo:** `aprendizados.md` (na raiz da skill, `.ai/skills/google-ads-ratos/aprendizados.md`)
