...
LEGACY_ENV_PATH = os.path.join(os.getcwd(), ".env")
...
def cmd_check():
    ...
    print()
    print("=== .env ===")
    if not os.path.isfile(ENV_PATH) and not os.path.isfile(LEGACY_ENV_PATH):
        print(f"  FALTA: Arquivo .env nao encontrado em {ENV_PATH}")
        print("  Crie o arquivo compartilhado com o template do SKILL.md")
        return
    status = check_env()
    print()
...
def is_port_free(port=OAUTH_PORT):
    """Verifica se a porta OAuth fixa esta livre."""
    try:
...
def run_oauth():
    ...
    if not is_port_free(port):
        print(f"ERRO: Porta {port} ocupada. Fecha o processo usando {SERVER}:{port} e tenta de novo.")
        sys.exit(1)
...
def save_refresh_token(token):
    """Salva o refresh token no .env compartilhado."""
