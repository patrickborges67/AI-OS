...
Checar se existe `.env (raiz do projeto)` na raiz do projeto. Se NAO existir, criar com o template:
...
O usuario precisa obter e preencher no `.env (raiz do projeto)` (tutorial completo em ratosdeia.com.br/assets/tutorial-token-google-ads/):
...
Depois que CLIENT_ID, CLIENT_SECRET e DEVELOPER_TOKEN estiverem no `.env (raiz do projeto)`, rodar:

```bash
python .claude/skills/google-ads-ratos/scripts/setup.py oauth
```

Isso abre o browser, o usuario autoriza, e o refresh token e salvo automaticamente no `.env (raiz do projeto)`. Sem copiar/colar nada.

**Ou rodar o fluxo completo de uma vez:**

```bash
python .claude/skills/google-ads-ratos/scripts/setup.py full
```
...
Depois que o `.env (raiz do projeto)` estiver preenchido e o teste passar, o Codex DEVE proativamente guiar o cadastro de contas:
