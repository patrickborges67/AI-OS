...
Checar se existe `.env (raiz do projeto)`. Se NAO existir, criar com o template:
...
O token fica isolado no `.env (raiz do projeto)` e nao vaza pra outras sessoes do terminal.
...
Depois que o `.env (raiz do projeto)` estiver preenchido e o `setup.py` passar, o Claude DEVE proativamente guiar o cadastro de contas:
...
| `pixels` | Lista pixels/datasets da conta | `read.py pixels --account act_123` |
| `pixel` | Detalhes de um pixel/dataset | `read.py pixel --id 123` |
| `pixel-stats` | Estatisticas de eventos do pixel/dataset | `read.py pixel-stats --id 123 --start-time 2026-04-01 --end-time 2026-04-30` |
| `custom-conversions` | Lista conversoes personalizadas | `read.py custom-conversions --account act_123` |
