...
- `OPENAI_API_KEY` existe no ambiente, `.env (raiz do projeto)`, `.env` local ou `~/.env`; nunca imprima o valor e nunca crie arquivo de segredo sem pedido explicito.
...
```bash
python .claude/skills/gpt-image-2/scripts/generate-image.py --prompt "..." --file caminho/saida.png --size 1024x1536 --quality medium
```

Edicao com referencia:

```bash
python .claude/skills/gpt-image-2/scripts/generate-image.py --prompt "..." --image ref.png --file saida.png --size 1024x1536 --quality medium
```
